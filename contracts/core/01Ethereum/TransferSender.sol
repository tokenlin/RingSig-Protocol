...
contract TransferSender is OwnerIsCreator, ITransferSender {
    ...
    function sendMessagePayLink(
        ...
        (, functionCallData) = dataCheckAndGetFunctionCallData(_nonce);
    ...
    function sendMessagePayNative(
        uint256 _nonce  
    ) external payable returns (bytes32 messageId, uint256 fees) {  // everyone can call this function
    ...
    function dataCheckAndGetFunctionCallData(
        ...
        bytes32[2] memory publicKey = IPool(pool).getDepositOrder(_nonce);
        require(publicKey[0] != bytes32(0x0));
        _arg = abi.encode(_nonce, publicKey);
        functionCallData = abi.encodeWithSignature("setDataFromTransferReceiver(bytes)", _arg);
    }
