...
contract Pool is IPool {
    ...
    function transfer(
        ...
        ISupervisor(supervision).transfer(_index, depositAmount);
        _deposit(_publicKey);

        if(ccipPayLink){
            ITransferSender(ccipSender).sendMessagePayLink(nonces);
        }else{
            ITransferSender(ccipSender).sendMessagePayNative{value: msg.value}(nonces);
        }
    }
